...
fn main() {
    ...
    for _ in 0..q {
        ...
        if t == 1 {
            ...
            let (mut l, r, mut a) = set.less_than(&(x, !0, !0)).unwrap();
            set.discard((l, r, a));
            cnt[a] -= r - l;
            a = c;
            set.insert((l, r, a));
            cnt[c] += r - l;
            if let Some((ll, rl, al)) = set.less_than(&(l, r, c)) {
                if al == a {
                    set.discard((ll, rl, al));
                    set.discard((l, r, a));
                    set.insert((ll, r, a));
                    l = ll;
                }
            }
            if let Some((lr, rr, ar)) = set.greater_than(&(l, r, c)) {
                if ar == c {
                    set.discard((lr, rr, ar));
                    set.discard((l, r, a));
                    set.insert((l, rr, a));
                }
            }
